...
export default function BlogHubClient({ initialBlogs }: { initialBlogs: any[] }) {
  ...
  useEffect(() => {
    const fetchBlogs = async () => {
      try {
        ...
        if (dbBlogs && !error && dbBlogs.length > 0) {
          ...
          setBlogs((current) => {
            ...
            if (JSON.stringify(current) !== JSON.stringify(combined)) {
               return combined;
            }
            return current;
          });
        ...
        console.warn("Client-side Supabase fetch failed for blogs", e);
      }
    };
    fetchBlogs();
  }, []);
    ...
    <>
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-10">
          {regularBlogs.map((blog: any, idx: number) => (
            <ScrollReveal key={blog.id || blog.slug} delay={idx * 100}>
